        ...
        for i in 0..worker_count.max(1) {
            ...
            workers.push(thread::spawn(move || {
                ...
                loop {
                    ...
                    if let Err(e) = copy_one_local_file(&job, &stats, &mut buffer) {
                        error!(
                            "Local copy worker {} failed: {:?} -> {:?}: {}",
                            i, job.src_path, job.dst_path, e
                        );
...
fn copy_one_local_file(
    ...
    if let Some(ref symlink_target) = job.meta.common.symlink_target_path {
        if let Some(parent) = job.dst_path.parent() {
            std::fs::create_dir_all(parent)?;
        }
        create_symlink(&job.dst_path, symlink_target)?;
        #[cfg(target_os = "linux")]
        {
            restore_xattrs(&job.dst_path, &job.meta.common.xattributes);
            restore_acl(
                &job.dst_path,
                &job.meta.common.posix_access_acl,
                &job.meta.common.posix_default_acl,
            );
        }
    ...
    let mut src = File::open(&job.src_path)?;
    stats.inc_src_opened();
    let mut dst = File::create(&job.dst_path)?;
...
fn create_symlink(dst_path: &PathBuf, target: &str) -> io::Result<()> {
    if dst_path.exists() {

...
#[cfg(feature = "nfs")]
use fpt::frame::backup_impls::{NfsSourceFileBackup, NfsSourceTargetFileBackup};
#[cfg(all(feature = "nfs", feature = "smb"))]
use fpt::frame::backup_impls::{NfsSourceSmbTargetFileBackup, SmbSourceNfsTargetFileBackup};
#[cfg(feature = "smb")]
use fpt::frame::backup_impls::{SmbFileBackup, SmbSourceFileBackup, SmbSourceTargetFileBackup};
#[cfg(feature = "nfs")]
use fpt::frame::NfsFileBackup;
use fpt::frame::{BackupConfig, DataLocation, FileBackup, LocalFileBackup};
...
fn run_backup(
    ...
    source: &DataLocation,
    target: &DataLocation,
) -> Result<fpt::frame::TransferStats, Box<dyn std::error::Error>> {
    match (source, target) {
...
fn parse_data_location(
    ...
    if spec.starts_with("smb://") || spec.starts_with(r"smb:\\") {
        #[cfg(feature = "smb")]
        {
            return Ok(DataLocation::smb(fpt::smb::SmbLocation::from_url(
                spec,
            )?));
        }
        #[cfg(not(feature = "smb"))]
...
fn print_summary(stats: &fpt::frame::TransferStats, elapsed: Duration) {

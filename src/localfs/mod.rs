...
//!
//! This module implements the blocking I/O (BIO) directory traversal for
//! local filesystem sources. It is the local counterpart to
//! [`crate::nfs::scanner`] and [`crate::smb::scanner`].
//!
//! ## Architecture
//!
//! - **Traversal workers** ([`bio`]): OS threads that pull directories from a
//!   shared [`SpillQueue`], enumerate entries via `std::fs`, and push
//!   [`DirBatchScanResult`] into the output [`BlockingQueue`].
//! - **Orchestrator** ([`Scanner`]): Enqueues root paths, spawns workers and
//!   metadata writers, and returns a [`RunningScan`] handle.

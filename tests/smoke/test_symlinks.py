...
Platform-aware: on Windows, requires symlink creation privilege (admin or Developer Mode).
"""
...
def test_symlinks_backup_succeeds(tmp_workspace: FptCli):
    ...
    if not links:
        pytest.skip("Symlink creation not supported on this filesystem/privilege level")
    ...
    assert (d_repo / "link_target_dir" / "inside.txt").exists(), (
        "Symlink target dir content missing"
    )
    ...
    for desc, link_path in links.items():
        ...
        try:
            link_exists = backup_link.exists(follow_symlinks=False)
        except PermissionError:
            # Directory symlinks may cause permission errors on Windows
            link_exists = backup_link.is_symlink()
        if desc == "broken":
            # broken symlinks may not be backed up — skip if missing
            if not link_exists:
                continue
        assert link_exists, f"Symlink missing in backup: {desc} ({rel})"
